...
const main = async () => {
    ...
    const adminDb        = await MongoClient.connect(url, options);
...
const _getStatusCount = (builds) => {
    ...
    return JSON.stringify(totals, null, 4);
};
...
const _getMaxParallelRunningCount = (events) => {
    ...
    return `${maxRunning} ${process.env.PRINT_MAX_DELAYED_BUILDS ? JSON.stringify(_.map(maxBuildIds, id => id), null, 2) : ''}`;
};
...
const _getAllBuilds = async () => {
    ...
    const cursor    = col.find(
        {
            "_id": { $gte: new ObjectId(Math.floor((new Date(`${FROM_DATE}T00:00:00.000+0000`).getTime()) / 1000).toString(16) + "0000000000000000"), ...(TO_DATE && {$lte: new ObjectId(Math.floor((new Date(`${TO_DATE}T00:00:00.000+0000`).getTime()) / 1000).toString(16) + "0000000000000000")}) }, ...(process.env.ACCOUNT_ID &&
                { "account": new ObjectId(process.env.ACCOUNT_ID) })
        },
        {
            _id: 1,
            created: 1,
